...
class StorageManager {
    ...
    class func uploadImage(_ image: UIImage, folder: StorageFolder, handler: @escaping (_ url: URL?, _ error: Error?) -> ()) {
        guard let data = image.jpegData(compressionQuality: 0.5) else { handler(nil, generateError("Please can not be null.")); return }
